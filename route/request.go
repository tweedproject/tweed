...
func (r *Request) BasicAuthorized(username, password, realm string) bool {
	u, p, given := r.Req.BasicAuth()
	if !given {
		r.Req.Header.Add("WWW-Authenticate", `Basic realm="`+realm+`"`)
		r.Fail(Unauthorized(nil, "you must first authenticate"))
		return false
	}
	...
	r.Fail(Forbidden(nil, "you must first authenticate with proper credentials"))
	return false
}
